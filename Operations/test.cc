...
void q2 () {
	...
	//char *pred_p = "(p_retailprice > 931.01) AND (p_retailprice < 935.3)";
...
void q5 () {
	...
	//clear_pipe(___ps,&__ps_sch,true);
...
void q6 () {
	...
	cout << " query6 \n";
	//char *pred_s = "(s_suppkey = s_suppkey)";
	char *pred_s = "(s_suppkey > 0)";
	init_SF_s (pred_s, 100);
	SF_s.Run (dbf_s, _s, cnf_s, lit_s); // 10k recs qualified

	//char *pred_ps = "(ps_suppkey = ps_suppkey)";
	char *pred_ps = "(ps_suppkey > 0)";
	init_SF_ps (pred_ps, 100);
	...
	int* whichAttr = new int[1];
	whichAttr[0] = 3;
	int* whichType = new int[1];
	whichType[0] = Int;
	grp_order.setNumAtts(1);
	grp_order.setWhichAtts(whichAttr);
	grp_order.setWhichType(whichType);
	grp_order.Print();
	delete[] whichAttr;
	delete[] whichType;
	...
	J.WaitUntilDone ();
//	int cnt = clear_pipe (_s_ps, &join_sch, true);
//	cout << " query6 returned join for " << cnt << endl;
//	G.WaitUntilDone ();

	Attribute grpbyatt[] = {DA,s_nationkey};
	Schema sum_sch ("sum_sch", 2, grpbyatt);
	int cnt = clear_pipe (_out, &sum_sch, true);
...
*/
	...
	cout << " query8 \n";
		...
		char *pred_p = "(p_partkey > 0)";
		init_SF_p (pred_p, 100);
		SF_p.Run (dbf_p, _p, cnf_p, lit_p); // 10k recs qualified
		...
		SF_ps.Run (dbf_ps, _ps, cnf_ps, lit_ps); // 161 recs qualified
		J.Run (_s, _ps, _s_ps, cnf_s_ps, lit_s_ps);
		CNF cnf_p_ps;
		Record lit_p_ps;
		get_cnf ("(p_partkey = ps_partkey)", p->schema(), ps->schema(), cnf_p_ps, lit_p_ps);
		Pipe j2(100);
		J2.Run(_s_ps,_p,j2,cnf_p_ps,lit_p_ps);
		G.Run (j2, _out, grp_order, func);
...
*/
	...
	Function func;
	char *str_sum = "(ps_partkey)";
	get_cnf (str_sum, ps->schema(), func);

	Schema om_schema("join_sch", 1, &IA);

	OrderMaker o(&om_schema);
	G.Run(_ps,outPipe,o,func);

	Attribute* attr = ps->schema()->GetAtts();
	Attribute* newAttr = new Attribute[3];
	newAttr[0] = IA;

	Schema out_sch ("out_sch", 1 , newAttr);
	clear_pipe(outPipe,&out_sch,true);
	SF_ps.WaitUntilDone ();
	G.WaitUntilDone ();

	cout << " My query8 finished.." << "\n";
}

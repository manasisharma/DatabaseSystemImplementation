...
void OrderMaker::setNumAtts(int _numAtts) {
	numAtts = _numAtts;
}

void OrderMaker::setWhichAtts(int* _whichAtts) {
	for (int i = 0; i < numAtts; i++) {
		whichAtts[i] = _whichAtts[i];
	}
}
void OrderMaker::setWhichType(int* _whichTypes) {
	for (int i = 0; i < numAtts; i++) {
		whichTypes[i] = (Type)_whichTypes[i];
	}
}

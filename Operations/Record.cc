...
//	cout <<"intial totspace = " << totSpace << endl;
...
//			cout <<"last totspace = " << totSpace << endl;
		} else {
			// in this case, subtract the start of the next field from the start of this field
			totSpace += ((int *) rec_bits)[attsToKeep[i] + 2] - ((int *) rec_bits)[attsToKeep[i] + 1];
//			cout <<"*not* last totspace = " << totSpace << endl;
		}
	}
	...
	if (bits == NULL)
	{
		cout << "MERGE RECORD:: ERROR : Not enough memory. EXIT !!!" << "trying to allocate = " << totSpace + 1 << endl;
		exit(1);
	}
...
//This is exactly same as above Print function just that it writes in ostream
